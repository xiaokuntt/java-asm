...
import cn.ykccchen.script.exception.StringLiteralException;
...
public class Tokenizer {
	...
	private static List<Token> tokenizer(CharacterStream stream, List<Token> tokens, boolean matchComment, String except) {
		...
		while (stream.hasMore()) {
			...
			if (stream.hasMore()) {
					ParseError.error(ScriptErrorCode.SCRIPT_TOKEN_ERROR, "Unknown token",
							stream.getSpan(stream.getPosition(), stream.getPosition() + 1), null);
			}
		}
		return tokens;
	}
	...
	private static boolean tokenizerLanguage(CharacterStream stream, List<Token> tokens) {
		if (stream.match("```", true)) {
			...
			if (stream.matchIdentifierStart(true)) {
				...
				stream.startSpan();
				if (!stream.skipUntil("```")) {
					ParseError.error(ScriptErrorCode.SCRIPT_TOKEN_ERROR, "```需要以```结尾",
							stream.endSpan(), new StringLiteralException());
				}
				tokens.add(new Token(TokenType.Language, stream.endSpan(-3)));
				return true;
			} else {
				ParseError.error(ScriptErrorCode.SCRIPT_TOKEN_ERROR, "```后需要标识语言类型",
						stream.endSpan(), new StringLiteralException());
			}
		}
		return false;
	...
	private static boolean tokenizerTemplateString(CharacterStream stream, List<Token> tokens, boolean matchComment) {
		if (stream.match("`", true)) {
			...
			if (!matchedEndQuote) {
				ParseError.error("模板字符串没有结束符`", stream.endSpan(), new StringLiteralException());
			}
		...
		if (stream.matchDigit(false)) {
			...
			if (stream.matchAny(true, "b", "B")) {
				if (type == TokenType.DoubleLiteral) {
					ParseError.error("Byte literal can not have a decimal point.", stream.endSpan());
				}
				type = TokenType.ByteLiteral;
			} else if (stream.matchAny(true, "s", "S")) {
				if (type == TokenType.DoubleLiteral) {
					ParseError.error("Short literal can not have a decimal point.", stream.endSpan());
				}
				type = TokenType.ShortLiteral;
			} else if (stream.matchAny(true, "L", "l")) {
				if (type == TokenType.DoubleLiteral) {
					ParseError.error("Long literal can not have a decimal point.", stream.endSpan());
				}
	...
	private static LiteralToken autoNumberType(Span span, int radix) {
		...
		if(text.length() == 2 && radix == 2) {
			return new LiteralToken(TokenType.ByteLiteral, span, (byte)0);
		}
		...
		if (stream.match(tokenType.getLiteral(), true)) {
			...
			while (stream.hasMore()) {
				...
				if (tokenType != TokenType.TripleQuote && (ch == '\r' || ch == '\n')) {
					ParseError.error(tokenType.getError() + tokenType.getError() + "定义的字符串不能换行", stream.endSpan(), new StringLiteralException());
				}
			}
			if (!matchedEndQuote) {
				ParseError.error("字符串没有结束符" + tokenType.getError(), stream.endSpan(), new StringLiteralException());
			}
	...
	private static boolean regexpToken(CharacterStream stream, List<Token> tokens) {
		...
		if (stream.match("/", false)) {
			...
			if (deep != 0) {
				ParseError.error("Missing ']'", stream.getSpan(maybeMissForwardSlash, maybeMissForwardSlashEnd - 1));
			}
			if (!matchedEndQuote) {
				stream.reset(mark);
				return false;
