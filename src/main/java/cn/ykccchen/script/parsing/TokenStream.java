...
public class TokenStream {
	...
	public Token expect(TokenType... types) {
		if (!match(true, types)) {
			...
			String typesStr= Stream.of(types).map(TokenType::getError).collect(Collectors.joining("','"));
			if (span == null) {
				ParseError.error(String.format("预计 [%s], 但是流已经遍历完毕",typesStr), this);
			} else {
				ParseError.error(String.format("预计 [%s], 实际 [%s]", typesStr, token.getText()), span);
			}
			return null;
		} else {
			return tokens.get(index - 1);
	...
	public Token expect(TokenType type) {
		if (!match(type, true)) {
			...
			Span span = token != null ? token.getSpan() : null;
			if (span == null) {
				ParseError.error("期待 '" + type.getError() + "', 但是流已经遍历完毕", this);
			} else {
				ParseError.error("期待 '" + type.getError() + "', 获得 '" + token.getText() + "'", span);
			}
			return null; // 执行不到这里
		} else {
			return tokens.get(index - 1);
	...
	public Token expect(String text, boolean ignoreCase) {
		...
		if (!result) {
			...
			Span span = token != null ? token.getSpan() : null;
			if (span == null) {
				ParseError.error("期待 '" + text + "', 但是流已经遍历完毕", this);
			} else {
				ParseError.error("期待 '" + text + "', 获得 '" + token.getText() + "'", span);
			}
