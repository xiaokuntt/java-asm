...
import cn.ykccchen.script.parsing.ast.binary.*;
...
public abstract class BinaryOperation extends Expression {
	...
	protected BinaryOperation(Expression leftOperand, Span span, Expression rightOperand) {
		super(span);
	...
	public static Expression create(Expression left, Token operator, Expression right) {
		if(operator.getType().isModifiable() && left instanceof VariableAccess && ((VariableAccess) left).getVarIndex().isReadonly()){
			ParseError.error("final修饰的变量不能被修改", new Span(left.getSpan(), right.getSpan()));
		}
		...
		switch (operator.getType()) {
			...
			case InstanceOf:
				...
				break;
			default:
				ParseError.error("[" + operator.getText() + "]操作符未实现", span);
		}
		return expression;
	}
