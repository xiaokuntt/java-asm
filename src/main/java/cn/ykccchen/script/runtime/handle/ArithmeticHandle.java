...
public class ArithmeticHandle {
	...
	public static Object divideAndRemainder(Float a, BigDecimal b) {
		return new BigDecimal(a.toString()).remainder(b);
	}

	public static Object divideAndRemainder(Float a, BigInteger b) {
		return new BigDecimal(a.toString()).remainder(new BigDecimal(b));
	}
	/* float % 操作实现 */
	...
	public static Object divideAndRemainder(BigDecimal a, Float b) {
		return a.remainder(new BigDecimal(b.toString()));
	}
	...
	public static Object divideAndRemainder(BigInteger a, Float b) {
		return new BigDecimal(a).remainder(new BigDecimal(b.toString()));
	}
	...
	private static Object reject(Object a, Object b, String symbol) {
		throw new ScriptRuntimeException(ScriptErrorCode.SCRIPT_OPERATOR_ERROR,
				String.format("`%s` 运算不支持 (%s,%s) 类型", symbol, typeName(a), typeName(b)));
	}
