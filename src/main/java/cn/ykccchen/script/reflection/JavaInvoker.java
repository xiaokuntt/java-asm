...
public class JavaInvoker<T extends Executable> {
	...
	JavaInvoker(T executable) {
		...
		if (!Modifier.isPublic(executable.getModifiers())
			...
			try {
				...
				// Strongly encapsulated JDK modules may reject deep reflection.
				// Invocation will still be attempted and report a useful access error.
			}
		}
	...
	public JavaInvoker<T> copy(){
		throw new ScriptRuntimeException(ScriptErrorCode.SCRIPT_UNSUPPORTED_OPERATION,
				"JavaInvoker subclass must implement copy()");
	}
	...
	Object invoke(Object target, Object... arguments) throws InvocationTargetException, IllegalAccessException, InstantiationException {
		throw new ScriptRuntimeException(ScriptErrorCode.SCRIPT_UNSUPPORTED_OPERATION,
				"JavaInvoker subclass must implement invoke()");
	}
